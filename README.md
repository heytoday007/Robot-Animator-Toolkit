...
Blender 插件：面向 URDF 机器人的**骨架合并、网格重绑定与动态 CSV/NPZ 导出**，支持从 URDF/SMURF 导入到动画数据导出的完整流程。
...
3. **导出数据**：在 **Dynamic CSV - Joint List** 里刷新关节列表并配置后，点击 **导出动态 CSV** 或 **导出动态 NPZ**。
...
| **导出动态 CSV / NPZ** | 按关节列表与帧范围导出每帧关节旋转值，可导出为 CSV 或 NPZ，并支持**角度**或**弧度**。 |
...
8. **导出动态 CSV / NPZ**  
   ...
   - 点击 **导出动态 CSV** 或 **导出动态 NPZ**，选择保存路径即可。
